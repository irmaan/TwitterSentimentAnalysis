...
def calculate_accuracy(predictions):
    correct=0
    datafile = open('test_targets.csv', 'r', newline='\n')
    dataReader = csv.reader(datafile)
    testData = list(dataReader)
    for i in range(len(testData)):
        if int(testData[i][0])==predictions[i][1]:
            correct+=1
...
if __name__ == '__main__':
    ...
    utils.save_results_to_csv(predictions, 'predictions.csv')
    testAcc=calculate_accuracy(predictions)
    print ('\nSaved to predictions.csv')
    print('\n Prediction Accuracy : %d',testAcc )
